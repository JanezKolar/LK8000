...
unsigned long		  TICKER_PFLX4 = 0;
unsigned long		  AlttimeOutTicker = 0;
...
BOOL DevLXMiniMap::Install(PDeviceDescriptor_t d)
{
  ...
  d->OnSysTicker = LXMiniMapOnSysTicker;
...
BOOL DevLXMiniMap::LXMiniMapOnSysTicker(DeviceDescriptor_t *d) {
	   ...
	   if(abs(GetTickCount() - TICKER_PFLX4) > 2000)
	   {
		   TICKER_PFLX4 = GetTickCount();
		   SendPFLX4(d);

	    	if(abs(GetTickCount() - TICKER) > 20000)
	    	{
	    		TICKER = GetTickCount();
	    		TCHAR mcbuf[100];
...
double QNEAltitudeToStaticPressure(double alt) {
  ...
  const double k1=0.190263;
  const double k2=8.417286e-5;
  return 100.0*pow((pow(1013.25,k1)-k2*alt),1.0/k1);
